...
if TYPE_CHECKING:
    from .optimizer_v2_class import OptimizerV2
import pandas as pd
import numpy as np
import os
import sys
...
class OutputManager(InitMixin):
    ...
    def __init__(self, optimizer: OptimizerV2) -> None:
 ...
 #               )
    ...
    def convert_idx_to_name(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        # If arc index exists, project to (dep_node, arr_node) columns for readability
        if "arc" in df.columns:
            # create columns if missing
            if "dep_node" not in df.columns:
                df["dep_node"] = pd.NA
            if "arr_node" not in df.columns:
                df["arr_node"] = pd.NA
            # build mapping from arc id to (dep,arr)
            nb = self.optimizer._model_builder._network_def
            arc2dep = {a.id: a.dep for a in nb.arc_list}
            arc2arr = {a.id: a.arr for a in nb.arc_list}
            # fill only rows that have arc defined
            mask_has_arc = df["arc"].notna()
            df.loc[mask_has_arc, "dep_node"] = df.loc[mask_has_arc, "arc"].map(arc2dep)
            df.loc[mask_has_arc, "arr_node"] = df.loc[mask_has_arc, "arc"].map(arc2arr)

        # Cast numeric-like indices to float for downstream name mapping
